...
#Source: https://stackoverflow.com/questions/16138744/extract-time-from-datetime-and-determine-if-time-not-date-falls-within-range
...
#Fill the phone list going page by page finding the sections and grabbing the information that is needed
#Source Used: https://www.crummy.com/software/BeautifulSoup/bs4/doc/
def FillPhoneList(lastPage):
    try:
        pageNumber = 1
        for pageNumber in range(int(3) + 1):#replace 3 with lastPage to grab all the pages
...
#Convert the ratings in the original list to a max of 10 and put into a new list
def ConvertRatings():
